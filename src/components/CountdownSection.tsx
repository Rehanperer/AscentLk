    ...
    return (
        <section id="countdown" className="relative overflow-hidden py-16 md:py-24 bg-atmospheric-blood">
            ...
            <div className="absolute inset-0 z-0">
                {/* Atmospheric crimson glow orbs — Mobile: reduced blur, Desktop: full */}
                <div className={`absolute top-1/4 left-0 w-full h-80 bg-[#4a0000]/40 rounded-full ${isMobile ? 'blur-[40px]' : 'blur-[120px]'} pointer-events-none`} />
                <div className={`absolute bottom-1/4 left-0 w-full h-80 bg-[#4a0000]/40 rounded-full ${isMobile ? 'blur-[40px]' : 'blur-[120px]'} pointer-events-none`} />
                {/* Large pulsing glow — Desktop only (DOM node removed on mobile, not just hidden) */}
                {!isMobile && (
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 items-center gap-12 lg:gap-20">
                    <div className="text-center lg:text-left">
                        {/* Mobile: Static rendering, no Framer Motion wrappers */}
                        {isMobile ? (
                            <>
                                <div className="inline-flex items-center gap-2 px-3 py-1 border border-[#ff4655]/30 bg-[#ff4655]/10 text-[#ff4655] font-mono text-xs mb-6 uppercase tracking-wider">
                                    <span className="w-1.5 h-1.5 rounded-full bg-[#00ff88]" />
                                    Status: ALLIANCE_PORTAL_ACTIVE
                                </div>

                                <h2 className="font-teko text-5xl md:text-7xl font-bold text-white leading-[0.9] mb-6 uppercase tracking-tight">
                                    REGISTRATION IS<br />
                                    <span className="text-[#ff4655] drop-shadow-[0_0_15px_rgba(255,70,85,0.4)]">OFFICIALLY LIVE</span>
                                </h2>

                                <p className="text-white/40 font-mono text-sm md:text-base max-w-md mx-auto lg:mx-0 leading-relaxed mb-10">
